...
const WIDTH = 1250,
  ...
  DAVE_SPEED = 300,
  JUMP_VELOCITY = 800,
  MIN_SPIN_VELOCITY = 100,
  MAX_SPIN_VELOCITY = 550,
  DRAG = 500,
  ANGULAR_DRAG = 150,
  MAX_BOOST = 200,
  IDLE_DELAY = 1000;
...
const START_COLOR = 0xbed5ff; // light blue
const MIDDLE_COLOR = 0x7da8e8; // medium sky blue
const END_COLOR = 0x201f4b; // deep night blue
...
class MainMenu extends Phaser.Scene {
  ...
  create() {
    ...
    var startChallengeText = this.add
    ...
    if (highScore > 0) {
      ...
      this.add
        ...
        .setActive(false);
      this.add
        .bitmapText(125, 150, "black-arial", highScore, 50)
        .setOrigin(0.5)
        .setScrollFactor(0)
        .setDepth(24)
        .setActive(false);
...
class DiveScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.runningStreak = this.add
      .bitmapText(125, 135, "black-arial", "streak: " + streak, 30)
      .setOrigin(0.5)
      .setScrollFactor(0)
      .setDepth(14)
      .setActive(false);
    this.runningScore = this.add
      .bitmapText(125, 75, "black-arial", "score: " + totalScore, 30)
      .setOrigin(0.5)
      .setScrollFactor(0)
      .setDepth(14)
      .setActive(false);
    ...
    this.gettingoutdave = this.add
      .sprite(WIDTH / 2, waterLevel - 150, "gettingoutdave")
      .setDepth(11);
  ...
  daveIsAboveBoard() {
    ...
    if (result) {
      // Reset all per-dive-attempt state — rotation accumulators AND tuck
      // state — so each bounce on the board starts a fresh dive attempt.
      // Without resetting tuckCount/tucked here, a re-bounce would carry the
      // previous attempt's tucks into scoring, subtracting (tuckCount - 1)
      // from each judge's score even though the new attempt only tucked once.
      this.sumRotation = 0;
      this.totalRotations = 0;
      this.previousAngle = 0;
      this.currentAngle = 0;
        ...
        setTimeout(() => {
          ...
          this.gettingoutdave.on(
            ...
            () => {
              ...
              this.climbdave.setVelocityY(-200); // Climb speed
            }
          );
        }, 1000);
      }
  ...
  calculateGameLogic() {
    const diveHeight = waterLevel - springboard.y;
    ...
    console.log("• Water Y:", waterLevel.toFixed(2));
  ...
  playerMobileMovementHandler() {
    ...
    if (
      this.mobileControls.leftButton.isDown ||
      this.mobileControls.rightButton.isDown ||
      this.mobileControls.jumpButton.isDown ||
      this.mobileControls.flipButton.isDown
    ) {
      if (this.diveComplete) {
        this.resetScene();
      } else {

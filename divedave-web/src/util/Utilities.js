...
// iPadOS 13+ Safari reports a macOS desktop UA, so the regexes miss it — but a
// Macintosh that also reports multi-touch is really an iPad. Without this, iPad
// users get the desktop keyboard hint and no on-screen walk buttons.
const IS_IPAD_DESKTOP_UA =
  /Macintosh/.test(navigator.userAgent) && navigator.maxTouchPoints > 1;

export const IS_MOBILE =
  MOBILE_USER_AGENT_RE_A.test(navigator.userAgent) ||
  MOBILE_USER_AGENT_RE_B.test(navigator.userAgent.substr(0, 4)) ||
  IS_IPAD_DESKTOP_UA;
...
export function fadeOutScene(sceneName, context, height) {
  ...
  context.time.addEvent({
    delay: 250,
    callback: () => context.scene.start(sceneName, { height }),
